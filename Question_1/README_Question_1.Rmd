...
Ranking the most popular baby names per decade
```{r}
popular_names <- Baby_names %>% 
    ...
    #Using my decade function to create a decade column and from there calculate the most popular names per decade. I do this in a genderless manner as famous names may be given to a boy or a girl(there are observations of even John being a female name)
    decade(., Year) %>% 
    mutate(Year = lubridate::year(Year)) %>% 
    group_by(Name, decade) %>%
    summarise(Count = sum(Count)) %>% 
    
    #I log the Count variable as there is a large differnece in the absoloute values for different names, which makes graphs extremely difficult to visualise
...
I attempted to create a ridgeline graph, however the counts were too big, and I could not find a way to scale each individual y-axis, so they did not provide much detail
```{r}
